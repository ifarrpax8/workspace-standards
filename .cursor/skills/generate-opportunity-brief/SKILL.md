...
- **Already have an Opp Brief** — proceed to [Generate PRD](../generate-prd/SKILL.md) instead
- **Feature is well-understood** and just needs refinement — use [Refine Ticket](../refine-ticket/SKILL.md) directly
- **Simple bug fix or enhancement** that doesn't need formal opportunity justification
...
Or with context:
```
@workspace-standards/.cursor/skills/generate-opportunity-brief/SKILL.md Generate an opportunity brief for a new payment method feature, targeting enterprise customers
```
...
- **Phase 1 (Context)**: Confirm the user provided at minimum: feature idea, target users, and strategic alignment. If any are missing, ask before proceeding.
- **Phase 3 (Draft)**: Confirm all template sections have content. The hypothesis should follow the if/then/because structure.
- **Phase 5 (Save)**: If saving to Confluence, verify the page was created. If saving as markdown, verify the file exists.

## Worked Example

**Input:** `Generate an opportunity brief for a new payment method feature, targeting enterprise customers`

**Key steps:**
1. Gathered context: feature = multi-payment-method support, users = enterprise partners, alignment = enterprise market expansion
2. Mapped to Engineering Codex facets: `payments`, `api-design`. Read `product.md` for payment flow patterns.
3. Read `gotchas.md` — flagged: PCI compliance scope expansion, payment provider failover handling
4. Drafted brief with hypothesis, projected ROI, and risk mitigations

**Output excerpt:**
```markdown
## Hypothesis

**If** we add support for ACH and wire transfer payment methods,
**then** enterprise adoption will increase by 15% within 6 months,
**because** 40% of enterprise prospects cite single-payment-method as a blocker.

## Key Risks (from Codex Gotchas)
- **PCI compliance scope expansion** — Adding payment methods may require PCI re-certification
- **Provider failover** — Need fallback strategy if primary payment processor is unavailable
```
