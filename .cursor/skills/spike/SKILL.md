...
- **Quick codebase exploration** (hours, no Jira deliverable) — use [Technical Deep Dive](../technical-deep-dive/SKILL.md)
- **Implementing a known solution** — use [Implement Ticket](../implement-ticket/SKILL.md)
- **Documenting a decision already made** — use [Generate ADR](../generate-adr/SKILL.md)
- **Simple question** that can be answered by reading code — just explore directly
...
**Input:** `Spike HRZN-456 — investigating caching options for currency rates`

**Key steps:**
1. Fetched HRZN-456 — spike for evaluating caching strategies for exchange rate API calls
2. Success criteria: answer "Which caching layer?" and "What TTL?" with confidence
3. Checked Engineering Codex `facets/caching/options.md` — found Redis and in-memory options pre-evaluated
4. Searched Pax8 org: `org:pax8 redis cache rate` — found existing Redis implementation in `payment-service`
5. Evaluated: Redis (shared, consistent, infra cost) vs Caffeine (in-process, simple, no infra)
6. Recommendation: Caffeine with 5-min TTL (single service, no cross-service sharing needed). High confidence.
