...
- **Formal time-boxed research** with Jira deliverables — use [Spike](../spike/SKILL.md)
- **Reviewing a PR** against team standards — use [Code Review](../code-review/SKILL.md)
- **Already know what to implement** — use [Implement Ticket](../implement-ticket/SKILL.md) directly
- **Simple file search** — just use Grep/Glob/Read directly
...
- **Phase 1 (Context)**: Confirm the target repository exists in the workspace. If not found, list available repos before proceeding.
- **Phase 2 (Exploration)**: Confirm at least one relevant file was found. If zero results, broaden the search or ask the user to clarify the topic.
- **Phase 3 (Patterns)**: Confirm the golden path file was loaded. If it doesn't exist, note which standards were unavailable in the output.
- **Phase 5 (Confidence)**: Present the updated confidence score and ask the user to confirm it reflects their understanding.

## Worked Example

**Input:** `Technical deep dive in currency-manager: How are exchange rates fetched and cached?`

**Key steps:**
1. Explored `currency-manager/src/main/kotlin/` — found `ExchangeRateService.kt`, `RateApiClient.kt`, `RateCacheConfig.kt`
2. Loaded `kotlin-spring-boot.md` golden path. Checked Engineering Codex `facets/caching/best-practices.md`.
3. Pattern analysis: rates fetched via `RateApiClient` (WebClient), cached in-memory via `@Cacheable` with 5-min TTL, no Redis
4. Found reference implementation: `RateCacheConfig.kt` configures Caffeine cache
5. Confidence updated: 1/3 → 3/3 — clear pattern, well-documented

**Output excerpt:**
```markdown
## Technical Deep Dive: Exchange rate fetching and caching

### Code Locations to Modify
| File | Purpose | Change Type |
|------|---------|-------------|
| `src/.../service/ExchangeRateService.kt` | Rate fetching with @Cacheable | Existing |
| `src/.../config/RateCacheConfig.kt` | Caffeine cache config (5-min TTL) | Existing |

### Patterns to Follow
**Reference:** `ExchangeRateService.kt` uses `@Cacheable("rates")` with Caffeine
```
