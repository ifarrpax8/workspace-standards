...
- **No approved Opportunity Brief** — use [Generate Opportunity Brief](../generate-opportunity-brief/SKILL.md) first
- **Already have a PRD** and need to break down into stories — jump to story breakdown in [Idea to Implementation](../idea-to-implementation/SKILL.md)
- **Simple feature** that doesn't warrant a formal PRD — just refine the ticket directly with [Refine Ticket](../refine-ticket/SKILL.md)

## Invocation

```
@workspace-standards/.cursor/skills/generate-prd/SKILL.md Generate a PRD from this opportunity brief: [paste or reference]
```
...
- **Phase 1 (Brief)**: Confirm the Opportunity Brief has been loaded and key sections (problem statement, hypothesis, target users) are non-empty. If critical sections are missing, flag before proceeding.
- **Phase 4 (Draft)**: Confirm all PRD template sections have content — no empty sections should remain. If a section can't be filled, add an explicit `[NEEDS INPUT]` marker.
- **Phase 6 (Save)**: If saving to Confluence, verify the page was created. If saving as markdown, verify the file exists.

## Worked Example

**Input:** `Generate PRD for bulk invoice PDF download feature`

**Key steps:**
1. Loaded approved Opp Brief — bulk PDF download for partner admins, enterprise customer focus
2. Mapped to Engineering Codex facets: `file-generation`, `async-processing`, `api-design`
3. Checked Pax8 standards: S3 for file storage (mandated), async job pattern (guidance)
4. Drafted PRD with 3 must-haves (API endpoint, PDF generation, async download), 1 nice-to-have (zip bundling)
5. Identified spike candidate: PDF generation at scale (>100 invoices)
...
### Must Have
- API endpoint to request bulk PDF generation for selected invoices
- Async PDF generation service using S3 storage (Pax8 standard)
- Download link with expiry delivered via notification

### Spike Candidates
- **PDF generation at scale** — Need to verify performance with >100 invoices per batch
```
